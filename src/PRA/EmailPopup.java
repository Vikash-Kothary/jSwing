...
public class EmailPopup extends jFrame {
	...
	public EmailPopup(final StudentList mainList) {
		...
		students.getScrollPanel("scrollPanel").setLayout(
				new GridLayout(mainList.size(), 1));
		...
		previewButtons.getButton("Send").addActionListener(
				new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							message.setFrom(new InternetAddress(EmailSettings
									.getUserName()));
							message.addRecipient(Message.RecipientType.TO,
									new InternetAddress(
											"tobyrbirkett@gmail.com"));
